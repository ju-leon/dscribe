        ...
        Args:
            ...
            average (str): The averaging mode over the centers of interest.
                Valid options are:
                    ...
                    * "off": No averaging.
                    * "inner": Averaging over atoms before summing up the magnetic quantum numbers: :math:`p_{nn'l}^{Z_1,Z_2} \sim \sum_m (\\frac{1}{n} \sum_i c_{nlm}^{i, Z_1})^{*} (\\frac{1}{n} \sum_i c_{n'lm}^{i, Z_2})`
                    * "outer": Averaging over the power spectrum of different atoms: :math:`p_{nn'l}^{Z_1,Z_2} \sim \\frac{1}{n} \sum_i \sum_m (c_{nlm}^{i, Z_1})^{*} (c_{n'lm}^{i, Z_2})`
